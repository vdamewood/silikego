...
void OnCalculate(HWND hwnd)
{
	...
	char* inString = (char*)GlobalAlloc(GPTR, ExpressionSize);
	GetDlgItemText(hwnd, CALCULATOR_INPUT, inString, ExpressionSize);

	std::unique_ptr<Silikego::SyntaxTreeNode> Node = Silikego::ParseInfix(
		std::unique_ptr<Silikego::DataSource>(new Silikego::StringSource(inString)));
	GlobalFree((HANDLE)inString);

	Silikego::Value Value = Node->Evaluate();

	std::string valueString = Value.ToString();
	SetDlgItemText(hwnd, CALCULATOR_OUTPUT, valueString.c_str());
}
